  ...
  resolutionTime: publicQuery.query(async () => {
    ...
    for (const category of categories) {
      const avgTime = await db
        .select({
          avg: sql<number>`COALESCE(AVG((julianday(resolvedAt) - julianday(createdAt)) * 24), 0)`,
        })
        .from(tickets)
  ...
  summary: publicQuery
    ...
    .query(async ({ input }) => {
      ...
      const totalTickets = await db
        .select({ count: sql<number>`count(*)` })
        .from(tickets)
        .where(sql`createdAt >= datetime('now', ${since})`);

      const resolvedTickets = await db
        .select({ count: sql<number>`count(*)` })
        .from(tickets)
        .where(
          sql`status = 'resolved' AND createdAt >= datetime('now', ${since})`
        );
